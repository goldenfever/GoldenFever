...
int nStakeTargetSpacing = 90; // 1 block - 90 seconds
...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval) {
        if (!pindexNext) {
            ...
            LogPrintf("GetKernelStakeModifier: run out of blocks in the active chain at block %d, exit and accept the modifier anyway\n", pindexNext->nHeight);
            break;
        }
...
{
    //Gfr will hash in the transaction hash and the index number in order to make sure each hash is unique
