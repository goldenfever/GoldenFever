...
// Copyright (c) 2017-2018 The Gfr developers
...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total gfr balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zGFRsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zGFR denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zGFR denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zGFR denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zGFR denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zGFR denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zGFR denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zGFR denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zGFR denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zGFR denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in gfr/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in gfr/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zgfrObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zgfrObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zGFRsupply", zgfrObj));
...
Value spork(const Array& params, bool fHelp)
{
    if (params.size() == 1 && params[0].get_str() == "show") {
        Object ret;
        for (int nSporkID = SPORK_STGFR; nSporkID <= SPORK_END; nSporkID++) {
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"gfraddress\"\n"
            "\nReturn information about the given gfr address.\n"
            "\nArguments:\n"
            "1. \"gfraddress\"     (string, required) The gfr address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"gfraddress\", (string) The gfr address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Gfr address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are gfr addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) gfr address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"gfraddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"gfraddress\"  (string, required) The gfr address to use for the signature.\n"
