...
//
// Bootup the Masternode, look for a 10000 GFR input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (service.GetPort() != MASTERNODE_PORT) {
                notCapableReason = strprintf("Invalid port: %u - only %d is supported on mainnet.", service.GetPort(), MASTERNODE_PORT);
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
                return;
            }
        } else if (service.GetPort() == MASTERNODE_PORT) {
            notCapableReason = strprintf("Invalid port: %u - %d is only supported on mainnet.", service.GetPort(), MASTERNODE_PORT);
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
            return;
        }
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (service.GetPort() != MASTERNODE_PORT) {
            errorMessage = strprintf("Invalid port %u for masternode %s - only %d is supported on mainnet.", service.GetPort(), strService, MASTERNODE_PORT);
            LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
            return false;
        }
    } else if (service.GetPort() == MASTERNODE_PORT) {
        errorMessage = strprintf("Invalid port %u for masternode %s - %d is only supported on mainnet.", service.GetPort(), strService, MASTERNODE_PORT);
        LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
        return false;
    }
