    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Gfr and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 GFR per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeGfrAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100GFR+1 ( bit on if present )
    // bit 1 - 10GFR+1
    // bit 2 - 1GFR+1
    // bit 3 - .1GFR+1
    // bit 3 - non-denom
...
bool CObfuScationSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey)
{
    ...
    if (GetTransaction(vin.prevout.hash, txVin, hash, true)) {
        BOOST_FOREACH (CTxOut out, txVin.vout) {
			LogPrint("masternode", "IsVinAssociatedWithPubkey: amount is %f GFR\n", out.nValue / COIN);
            if (out.nValue == mnActivateAmount || out.nValue == mnActivateSurvive) {
				LogPrint("masternode", "IsVinAssociatedWithPubkey: amount is correct\n");
                if (out.scriptPubKey == payee2) {
					LogPrint("masternode", "IsVinAssociatedWithPubkey: payee is correct\n");
					return true;
				} else
					LogPrint("masternode", "IsVinAssociatedWithPubkey: payee is NOT correct\n");
				
            } else
   				LogPrint("masternode", "IsVinAssociatedWithPubkey: amount is NOT correct\n");
