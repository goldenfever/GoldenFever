...
#if defined(NDEBUG)
#error "Gfr cannot be compiled without assertions."
#endif
...
map<COutPoint, int> mapStakeSpent;  //fake PoS fix
...
unsigned int nStakeMinAge = 12 * 60 * 60; //12 hours RELEASE
//unsigned int nStakeMinAge = 4 * 60 * 60; //4 hours FAST TRACK TEST!!!
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zGfr spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zGfr spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zGfr spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 100000 * COIN;  //premine 100000
    } else if(nHeight >=2 && nHeight <= Params().LAST_POW_BLOCK()) { //PoW phase
        nSubsidy = 0.9 * COIN;
    } else if(nHeight >Params().LAST_POW_BLOCK() && nHeight < 10000) { //PoS phase
        nSubsidy = 0.9 * COIN;
        ...
        nSubsidy = 5.2 * COIN;
    } else if(nHeight >= 100000 && nHeight < 140000) {
        nSubsidy = 5.3 * COIN;
    } else if(nHeight >= 140000 && nHeight < 160000) {
        nSubsidy = 5.5 * COIN;
    } else if(nHeight >= 160000 && nHeight < 180000) {
        nSubsidy = 6 * COIN;
        ...
        nSubsidy = 6 * COIN;
    } else if(nHeight >= 1000000 && nHeight < 1300000) {
        nSubsidy = 4 * COIN;
    } else if(nHeight >= 1300000 && nHeight < 1600000) {
        nSubsidy = 3 * COIN;
    } else if(nHeight >= 1600000 && nHeight < 2000000) {
        nSubsidy = 2 * COIN;
    }    
    else if(nHeight >= 2000000) {
        int nMul = (nHeight-2000000)/2000000;
        if(nMul < 32)
			nSubsidy = (2.0 / pow(2,nMul)) * COIN;
		else
			nSubsidy = 0; //down to zero
    } else {
        nSubsidy = 0.5 * COIN;  //everything else
    }
    return nSubsidy;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Gfr
...
{
    RenameThread("gfr-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZGFRMinted()
{
...
void RecalculateZGFRSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zGFR supply
...
bool RecalculateGFRSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zGfr with serial %s is already in the block %d\n",
    ...
    //LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zGfrSpent: %s\n",
    ...
    //int nExpectedHeight = pindex->pprev->nHeight+1;
    //LogPrintf("ConnectBlock: check expected mint for height %d\n",nExpectedHeight);
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert GFR to zGFR
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.vtx.empty() || block.vtx.size() > nMaxBlockSize/50 || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION) > nMaxBlockSize)
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zGfr spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zGfr serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zGfr mints and %d zGfr spends\n", __func__, nMints, nSpends);
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos* dbp)
{
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                // locate a header
                unsigned char buf[MESSAGE_STGFR_SIZE];
                blkdat.FindByte(Params().MessageStart()[0]);
                nRewind = blkdat.GetPos() + 1;
                blkdat >> FLATDATA(buf);
                if (memcmp(buf, Params().MessageStart(), MESSAGE_STGFR_SIZE))
                    continue;
                // read size
                blkdat >> nSize;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Gfr: We use certain sporks during IBD, so check to see if they are
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (memcmp(msg.hdr.pchMessageStart, Params().MessageStart(), MESSAGE_STGFR_SIZE) != 0) {
            LogPrintf("PROCESSMESSAGE: INVALID MESSAGESTGFR %s peer=%d\n", SanitizeString(msg.hdr.GetCommand()), pfrom->id);
            fOk = false;
            break;
        }
