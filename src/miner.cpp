...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (!pblocktemplate.get()) {
        LogPrintf("DEBUG CreateNewBlock: Cannot create a new block template\n");
        return NULL;
	}
    ...
    if (fProofOfStake) {
        ...
        if (!fStakeFound) {
             LogPrintf("DEBUG CreateNewBlock: Stake not found\n");
             return NULL;
		}
    }
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zGfr spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zGfr serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        CValidationState state;
        if (!TestBlockValidity(state, *pblock, pindexPrev, !fProofOfStake, false)) {
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GfrMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("GfrMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("GfrMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gfr-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
		...
		//check to stop PoW miner when PoS starts
        ...
        LogPrintf("Running GfrMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
