                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGFR ought to be enough for anybody." - Bill Gates, 2017
    ui->zGFRpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGFRSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzGFR->setEnabled(false);
        ui->pushButtonMintzGFR->setToolTip(tr("zGFR is currently disabled due to maintenance."));

        ui->pushButtonSpendzGFR->setEnabled(false);
        ui->pushButtonSpendzGFR->setToolTip(tr("zGFR is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGFRpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGFR_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGFR is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGFR...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGFR in ") +
...
void PrivacyDialog::on_pushButtonSpendzGFR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGFR is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zGFR
        sendzGFR();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGFR
    sendzGFR();
}

void PrivacyDialog::on_pushButtonZGfrControl_clicked()
{
    ZGfrControlDialog* zGfrControl = new ZGfrControlDialog(this);
    zGfrControl->setModel(walletModel);
    zGfrControl->exec();
}

void PrivacyDialog::setZGfrControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGfrSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGFR()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Gfr Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGFRpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGFR is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GFR </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGFR</b>";
    ...
    // use mints from zGfr selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGfrControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZGfrControlDialog::GetSelectedMints();
    }
    ...
    // Clear zgfr selector in case it was used
    ZGfrControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGfr Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Gfr, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGfr Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGFR </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGFR "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGFR "));
    ui->labelzGFRAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzGFRSyncStatus->setVisible(fShow);
}
