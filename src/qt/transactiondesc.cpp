        ...
        if (signatures >= 0) {
            if (signatures >= SWIFTTX_SIGNATURES_REQUIRED) {
                ...
                if (nDepth < 0)
                    ...
                    return tr("%1/ofgfr (verified via SwiftX)").arg(nDepth);
                else if (nDepth < 6)
                ...
                if (!wtx.IsTransactionLockTimedOut()) {
                    ...
                    if (nDepth < 0)
                        ...
                        return tr("%1/ofgfr (SwiftX verification in progress - %2 of %3 signatures)").arg(nDepth).arg(signatures).arg(SWIFTTX_SIGNATURES_TOTAL);
                    else if (nDepth < 6)
                    ...
                    if (nDepth < 0)
                        ...
                        return tr("%1/ofgfr (SwiftX verification failed)").arg(nDepth);
                    else if (nDepth < 6)
...
QString TransactionDesc::toHTML(CWallet* wallet, CWalletTx& wtx, TransactionRecord* rec, int unit)
{
    ...
    // Message from normal gfr:URI (gfr:XyZ...?message=example)
