...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::GFR, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (gfr.conf).");
            QMessageBox::warning(this, "GFR Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
void BlockExplorer::setContent(const std::string& Content)
{
	...
	//color for blockexplorer
    QString CSS = "body {font-size:12px; color:white; bgcolor:#360514;}\n a, span { font-family: monospace; }\n span.addr {color:#dddddd; font-weight: bold;}\n\
table tr td {padding: 3px; border: 1px solid black; background-color: #360514;}\n td.d0 {font-weight: bold; color:white;}\n h2, h3 { white-space:nowrap; color:white;}\n\
a { color:#ffffff; text-decoration:none; }\n a.nav {color:#dddddd;}\n";
    QString FullContent = "<html><head><style type=\"text/css\">" + CSS + "</style></head>" + "<body bgcolor=#360514>" + Content.c_str() + "</body></html>";
