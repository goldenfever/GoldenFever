...
#include "ui_zgfrcontroldialog.h"
...
std::list<std::string> ZGfrControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGfrControlDialog::listMints;

ZGfrControlDialog::ZGfrControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGfrControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGfrControlDialog::~ZGfrControlDialog()
{
    delete ui;
}

void ZGfrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGfrControlDialog::updateList()
{
...
void ZGfrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGfrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGfr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGfrControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGfrControlDialog::GetSelectedMints()
{
...
void ZGfrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
